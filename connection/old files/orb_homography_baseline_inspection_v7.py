#!/usr/bin/env python3
"""orb_homography_baseline_inspection_v7.py

v7: Reduce false BAD due to small gap jitter (0-7 px)
----------------------------------------------------
You observed the seated connector showing Gap bouncing between 0 and ~7, which can
trip BAD when GAP_FAIL_AT_OR_ABOVE_PX=4.

Root cause
----------
- The instantaneous gap estimate is noisy (lighting, edge competition).
- With a low fail threshold (4 px) and fail debounce (2 frames), brief spikes can trip BAD.
- If PER_PART_LATCH is enabled, once BAD it stays BAD until part leaves or reset.

v7 solution
-----------
1) Use a *median-filtered gap* for decisions:
   - Maintain a small deque of recent gap values (GAP_HISTORY = 7).
   - Use the median (gap_med) to decide fail/pass.
2) Only evaluate fail/pass when the boundary signal is "stable enough":
   - Require stable_count >= STABLE_REQUIRED_FOR_DECISION (default 2).
   - This prevents decisions during boundary hunting.
3) Keep per-part latch behavior, but reduce false trips with the above filters.
4) Keep baseline capture session method from v5/v6.
5) Keeps toggles:
   - m toggles PerPartLatch
   - k toggles ClearOnReseat (only relevant when PerPartLatch=False)
   - r reset
   - u clears baseline AND resets state
...
FAIL_CONSEC_TO_TRIP = 3          # was 2; slightly more robust
PASS_CONSEC_TO_CLEAR = 6

# Decision filtering
GAP_HISTORY = 7                  # median over last 7 gap samples
STABLE_REQUIRED_FOR_DECISION = 2 # require some stability before deciding
...
def run(camera, converter, cfg):
    ...
    def reset_part_state(reason=""):
        ...
        nonlocal boundary_hist, prev_boundary, stable_count, gap_hist
    ...
    while camera.IsGrabbing():
        ...
        if state == "INSPECT" and use_H is not None:
            ...
            if seam_patch is not None:
                ...
                if baseline is not None and boundary_med is not None:
                    ...
                    if len(gap_hist) >= 3:
                        gap_med = int(np.median(np.array(gap_hist, dtype=np.int32)))
                    else:
                        gap_med = gap_px
        ...
        # Decision logic uses gap_med and stability requirement
        if state == "INSPECT" and not paused and baseline is not None:
            if gap_med is None:
                if FAIL_IF_BOUNDARY_MISSING:
                    bad = True
            else:
                # Only decide when boundary is somewhat stable
                if stable_count >= STABLE_REQUIRED_FOR_DECISION:
        ...
        cv2.putText(bg, f"DecisionStableReq: {STABLE_REQUIRED_FOR_DECISION}  FailDebounce: {FAIL_CONSEC_TO_TRIP}", (30, 415), FONT, 0.75, txt, 2)
        ...
        if dbg_note:
            cv2.putText(bg, f"Boundary note: {dbg_note}", (30, 555), FONT, 0.75, txt, 2)
