#!/usr/bin/env python3
"""orb_homography_baseline_inspection_v10.py

v10: Fix "after baseline it immediately goes RED" when connector is seated
-----------------------------------------------------------------------
Your screenshot shows, immediately after baseline capture:
  BaselineX ~315, BoundaryMed ~323-324, GapMed ~8-9, causing FAIL>=4 to trip.
That indicates the baseline capture latched one edge, but post-baseline tracking latched
another nearby edge.

v10 changes
----------
1) Baseline capture is now TRACKED (sticky):
   - First sample finds a boundary in the pre-baseline search window.
   - Subsequent samples search near the previous sample (tracking) so the same physical
     edge is followed during the capture session.
   - Baseline is the median of tracked samples.

2) After baseline capture, we set prev_boundary = baseline to anchor tracking immediately.

3) Add a short "grace period" after baseline capture where decisions are ignored
   (prevents an immediate trip during settling): POST_BASELINE_GRACE_FRAMES.

4) Narrow tracking window to reduce edge hopping:
   - TRACK_NEAR_PX default 6 (was 10).

5) Keeps median filtered gap decision, hard-fail path, latch toggles, and evidence saving.
...
# After baseline capture, ignore decisions for a short time (settling)
POST_BASELINE_GRACE_FRAMES = 20
...
def detect_boundary_x(seam_roi_gray, baseline=None, prev=None, track_override=False):
    """Detect boundary x.

    If baseline is None: uses wide prebaseline window.
    If baseline is set: uses baseline window and tracking target (prev or baseline).

    track_override=True is used during baseline capture to make capture sticky even
    ...
    if baseline is None:
        ...
        a = clamp(a, 0, w - 2)
        b = clamp(b, a + 2, w)
        target = prev if track_override else None
    else:
...
def main():
    ...
    try:
        ...
        baseline_samples = []
        capture_prev = None  # NEW: tracked capture previous

        post_baseline_grace = 0
        ...
        def reset_part_state(reason=""):
            ...
            nonlocal baseline_capture_active, baseline_capture_left, baseline_samples, capture_prev
            nonlocal post_baseline_grace
            bad = False
            part_failed = False
            fail_consec = 0
            pass_consec = 0
        ...
        while camera.IsGrabbing():
            ...
            if state == "INSPECT" and use_H is not None:
                ...
                if seam_patch is not None:
                    ...
                    # Baseline capture (tracked)
                    if baseline_capture_active:
                        baseline_capture_left -= 1
                        # Use tracked detection even though baseline is None
                        bx_cap, _ = detect_boundary_x(seam_patch, baseline=None, prev=capture_prev, track_override=True)
                        if bx_cap is not None:
                            capture_prev = bx_cap
                            baseline_samples.append(int(bx_cap))
                        ...
                        if baseline_capture_left <= 0:
                            ...
                            if len(baseline_samples) >= BASELINE_CAPTURE_MIN_SAMPLES:
                                ...
                                save_config(cfg)
                                # Anchor tracking at baseline immediately
                                prev_boundary = baseline
                                # Clear counters/history so we don't instantly trip
                                boundary_hist.clear(); gap_hist.clear()
                                fail_consec = pass_consec = hard_fail_consec = 0
                                bad = False; part_failed = False; saved_this_part = False
                                max_gap_stable = 0
                                stable_gap_count = stable_boundary_count = 0
                                post_baseline_grace = POST_BASELINE_GRACE_FRAMES
                                toast(f"Baseline captured X={baseline} (tracked) samples={len(baseline_samples)}")
                            else:
                                toast(f"Baseline capture failed: {len(baseline_samples)} samples")
                            baseline_samples.clear()
                            capture_prev = None
            ...
            if k in (ord('b'), ord('B')):
                if state != "INSPECT" or seam_patch is None:
                    ...
                    baseline_samples.clear()
                    capture_prev = None
                    toast("Baseline capture (tracked) started")

            # Countdown grace period
            if post_baseline_grace > 0:
                post_baseline_grace -= 1
            ...
            cv2.putText(bg, f"TrackNear={TRACK_NEAR_PX}px TrackMinConf={TRACK_MIN_CONF} Grace={post_baseline_grace}", (30, 335), FONT, 0.75, txt, 2)
