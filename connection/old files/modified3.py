#!/usr/bin/env python3
"""orb_homography_inspection_v25_gap_darkness_otsu_band_tuning.py

Production-tolerant connector seating inspection (dark-gap detection)
===================================================================

This version adds interactive **gap-band tuning hotkeys** so you can place the
measurement band exactly over the true interface gap line, without editing
code and restarting.

Why this is needed
------------------
Your readings (dark_ratio~0.8 and max_run~full width) while seated indicate the
band is covering a large naturally-dark region (shadow/hardware) instead of the
actual seam interface. Band tuning fixes this.

Detection method
----------------
- ORB + RANSAC homography locates the part.
- Warp seam ROI to a normalized seam patch.
- On a configurable band (Y0..Y1) inside the seam patch:
    * Apply Gaussian blur
    * Compute Otsu threshold (binary inverse => dark pixels are 1)
    * Compute:
        - dark_ratio: fraction of dark pixels in band
        - max_run: longest contiguous run of "gap columns" across width
...
Band tuning hotkeys
-------------------
- [  : move band UP (decrease y0/y1)
- ]  : move band DOWN (increase y0/y1)
- -  : shrink band height
- =  : grow band height
- s  : SAVE band fractions to vision_config.json (gap_band_y0_frac/y1_frac)
- o  : reset band to defaults (0.40..0.65)
...
# -------------------------- Gap band defaults (fractions of patch height) --------------------------
DEFAULT_Y0_FRAC = 0.40
DEFAULT_Y1_FRAC = 0.65

# Band tuning step sizes
SHIFT_STEP_FRAC = 0.02   # move up/down
SIZE_STEP_FRAC  = 0.02   # grow/shrink
MIN_BAND_HEIGHT = 0.04   # minimum band thickness

# -------------------------- Otsu params --------------------------
BAND_BLUR_K = 5  # odd
COL_DARK_RATIO_AT_OR_ABOVE = 0.55  # stricter column criterion to avoid false full-width runs
...
def compute_gap_metrics_otsu(seam_patch: np.ndarray, y0_frac: float, y1_frac: float):
    ...
    y0 = int(np.clip(y0_frac * h, 0, h - 1))
    y1 = int(np.clip(y1_frac * h, y0 + 1, h))
...
def main():
    ...
    # Load persisted band settings if present
    ...
    try:
        ...
        while cam.IsGrabbing():
            ...
            # Band grow/shrink around center
            if k == ord('-'):
            ...
            if k in (ord('s'), ord('S')):
                ...
                save_config(cfg)
                print(f"[BAND] Saved gap band: y0={y0_frac:.3f} y1={y1_frac:.3f}")
            ...
            cv2.putText(bg, f"Band y0={y0_frac:.3f} y1={y1_frac:.3f}  ([ ] move, -/= size, s save, o reset)", (30, 225), FONT, 0.75, txt, 2)

            if dark_ratio is not None:
                cv2.putText(bg, f"dark_ratio={dark_ratio:.3f} (fail>={DARK_RATIO_FAIL})", (30, 260), FONT, 0.85, txt, 2)
            else:
                cv2.putText(bg, "dark_ratio=None", (30, 260), FONT, 0.85, txt, 2)

            if max_run is not None:
                cv2.putText(bg, f"max_run={max_run} px (fail>={MAX_RUN_FAIL})", (30, 295), FONT, 0.85, txt, 2)
            else:
                cv2.putText(bg, "max_run=None", (30, 295), FONT, 0.85, txt, 2)
                ...
                if seam_patch is not None:
                    ...
                    if band_y is not None:
                        ...
                        if otsu_thr is not None:
                            cv2.putText(patch_vis, f"otsu_thr={otsu_thr:.0f}", (5, max(15, y0+18)), FONT, 0.6, (0, 255, 255), 2)
