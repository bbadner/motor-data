#!/usr/bin/env python3
"""orb_homography_baseline_inspection_v3.py

ORB + Homography + Baseline Boundary (v3)
---------------------------------------
This is the full updated runtime script to address the false-gap issue you observed
(seated connector but large computed gap).

What changed vs v2
------------------
1) Tightened the post-baseline boundary search window:
   - BASELINE_MARGIN_RIGHT default reduced to 60 px (was 240).
   This prevents the detector from "wandering" onto a different strong edge.
2) Baseline capture requires stability:
   - You can only capture baseline (b) when stable_count >= STABLE_REQUIRED.
   This prevents capturing baseline while the boundary is still hunting.
3) Debug overlay now shows the TEMPLATE ROI (ORB anchor) as a BLUE quad.
4) Keeps v2 robustness fixes:
   - Consistent seam quad ordering each frame.
   - Clears boundary history on baseline capture and baseline clear.
   - Larger jump rejection during bring-up (JUMP_REJECT_PX=25).

Inputs
------
- template.png
- vision_config.json created by calibrate_rois_orb.py
  - seam_quad_in_template: 4 points (tl,tr,br,bl) in TEMPLATE coordinates
  - warp_size: {w,h}
  - baseline_boundary_x: saved when you press 'b'

Keys
----
  b : capture baseline (requires stable_count)
  u : clear baseline
  l : toggle latch mode
  r : reset per-part state
  p : pause
  d : toggle debug window
  c : exit (recalibrate if needed)
  ESC : quit
...
# IMPORTANT: tightened from 240 -> 60 to avoid latching onto unrelated edges
BASELINE_MARGIN_RIGHT = 60
BOUNDARY_CONF_MIN = 1.0
BOUNDARY_HISTORY = 9
JITTER_PX_MAX = 3
STABLE_REQUIRED = 6
# Bring-up: allow bigger jumps; after validated, you can reduce to ~10
JUMP_REJECT_PX = 25
...
def run(camera, converter, cfg):
    ...
    print("Baseline capture requires stability: stable_count >= STABLE_REQUIRED")
    ...
    while camera.IsGrabbing():
        ...
        # Baseline capture (requires stability)
        if k in (ord('b'), ord('B')):
            if seam_patch is None:
                ...
                print(f"[BASELINE] not stable yet ({stable_count}/{STABLE_REQUIRED}); wait and press b again.")
            else:
                baseline = int(boundary_med)
                cfg["baseline_boundary_x"] = int(baseline)
                save_config(cfg)
                # Clear tracking after baseline capture
                boundary_hist.clear()
                prev_boundary = None
                stable_count = 0
        ...
        # Show boundary search window for clarity
        if baseline is not None:
            cv2.putText(bg, f"Search window: baseline-{BASELINE_MARGIN_LEFT} to baseline+{BASELINE_MARGIN_RIGHT}",
                        (30, 445), FONT, 0.75, txt, 2)

        if reason:
            cv2.putText(bg, f"ORB note: {reason}", (30, 480), FONT, 0.75, txt, 2)
        if dbg_note:
            cv2.putText(bg, f"Boundary note: {dbg_note}", (30, 515), FONT, 0.75, txt, 2)
        ...
        if show_debug:
            ...
            # Draw template quad (blue)
            if tpl_quad_frame is not None:
                pts = np.int32(tpl_quad_frame).reshape(-1, 1, 2)
                cv2.polylines(dbg_img, [pts], True, (255, 0, 0), 2)
                cv2.putText(dbg_img, "TEMPLATE (ORB)", (pts[0][0][0], max(20, pts[0][0][1] - 10)),
                            FONT, 0.7, (255, 0, 0), 2)

            # Draw seam quad (yellow)
            if seam_quad_frame is not None:
                pts = np.int32(seam_quad_frame).reshape(-1, 1, 2)
            ...
            if seam_patch is not None:
                ...
                # Scale inset a bit larger for easier viewing
                scale = 1.6
