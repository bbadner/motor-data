#!/usr/bin/env python3
"""orb_homography_inspection_v12_profile_shift_score_resid.py

Connector seating inspection using ORB homography + seam patch profile matching.

What's new vs v11_profile_shift_fixed:
- Signed shift search (-MAX..+MAX) instead of only 0..MAX.
- Adds MATCH QUALITY gates:
    * correlation score gate (fails when score drops below threshold)
    * aligned residual gate (fails when profile shape differs even if shift ~ 0)
- Optional robustness:
    * skip inspection if seam quad warps out of frame
    * template/seam sanity checks at startup
    * save fail snapshots on first BAD transition
...
Tune:
- FAIL_AT_OR_ABOVE_PX / HARD_FAIL_AT_OR_ABOVE_PX (shift-based)
- SCORE_FAIL_AT_OR_BELOW and RESID_FAIL_AT_OR_ABOVE (match-quality based)
...
# Match quality gates (these catch your "shift stays 0 but connector changed" case)
# Correlation score: closer to 1.0 = better match.
SCORE_FAIL_AT_OR_BELOW = 0.92
SCORE_CONSEC_TO_TRIP = 3

# Residual: mean absolute difference between aligned z-scored profiles (lower = better)
...
def save_fail_images(frame_gray: np.ndarray, annotated_bgr: np.ndarray, info_text: str = "") -> None:
    """Save raw + annotated frames when a failure is detected."""
    ensure_dir(FAIL_DIR)
    ts = stamp()
...
def build_profile(patch_gray: np.ndarray) -> np.ndarray | None:
    """Build a z-scored 1D profile from the seam patch."""
...
def best_shift_signed(base_prof: np.ndarray, cur_prof: np.ndarray, max_shift: int) -> tuple[int, float]:
    """Find signed shift (-max..+max) maximizing normalized dot product."""
    w = len(base_prof)
...
def aligned_residual(base_prof: np.ndarray, cur_prof: np.ndarray, shift: int) -> float:
    """Mean absolute difference between aligned profiles after applying signed shift."""
    w = len(base_prof)
    s = int(shift)
    if s >= 0:
        a = base_prof[: w - s]
...
def main():
    ...
    # Sanity-check seam quad against template bounds (prevents silent failures)
        ...
        while cam.IsGrabbing():
            ...
            if state == "INSPECT" and use_H is not None:
                ...
                if quad_in_frame(seam_quad_frame, fw, fh, margin=5):
                    ...
                    # Out-of-frame seam: treat as unavailable this frame
                    seam_patch = None
                    prof = None
            ...
            # Compute metrics (only update histories when we have baseline and a valid profile)
            if base_profile is not None and prof is not None and len(base_profile) == len(prof):
                ...
                # Update histories (optionally require fresh H to reduce weirdness on hold)
                if H_is_fresh:
            ...
            if state == "INSPECT" and (not paused) and base_profile is not None and shift_med is not None:
                ...
                # Optional clearing (kept from your original logic)
                if (
                    (not per_part_latch)
                    and clear_on_reseat
                ...
                if per_part_latch and ((shift_med is not None and shift_med >= FAIL_AT_OR_ABOVE_PX) or (score_med is not None and score_med <= SCORE_FAIL_AT_OR_BELOW) or (resid_med is not None and resid_med >= RESID_FAIL_AT_OR_ABOVE)):
                    part_failed = True
            ...
            cv2.putText(
                bg,
                f"ShiftFail>={FAIL_AT_OR_ABOVE_PX} HardShiftFail>={HARD_FAIL_AT_OR_ABOVE_PX}",
                (30, 275),
                FONT,
                0.75,
                txt,
            ...
            cv2.putText(
                bg,
                f"Score: {None if score is None else f'{score:.3f}'}  ScoreMed({SCORE_HISTORY}): {None if score_med is None else f'{score_med:.3f}'}  Fail<= {SCORE_FAIL_AT_OR_BELOW}",
                (30, 310),
                FONT,
                0.75,
                txt,
            ...
            cv2.putText(
                bg,
                f"Resid: {None if resid is None else f'{resid:.3f}'}  ResidMed({RESID_HISTORY}): {None if resid_med is None else f'{resid_med:.3f}'}  Fail>= {RESID_FAIL_AT_OR_ABOVE}",
                (30, 345),
                FONT,
                0.75,
                txt,
