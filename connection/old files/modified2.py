#!/usr/bin/env python3
"""orb_homography_inspection_v24_gap_darkness_otsu.py
...
Problem observed
----------------
In your disconnected test the system still showed GOOD because the computed
"dark" threshold was too low (e.g., thr=8). With an 8-bit image (0..255),
only near-black pixels are counted as gap, so a real gap that is "darker" but
not near-black may be missed.

This version switches the per-frame thresholding to Otsu on the gap band.
Otsu automatically selects a threshold that separates darker pixels from
brighter ones, making it far more tolerant to lighting / pose changes.

What remains the same
---------------------
- ORB + RANSAC homography locates the part.
- Seam ROI is warped into a normalized seam patch.
- We compute dark_ratio and max_run on a gap band.
- BAD is latched if PerPartLatch=True.
- Baseline capture still exists (optional) to store band stats for reference,
  but Otsu thresholding is used for detection.
...
- tracking lost,
- seam out of view,
- empty seam patch/band.

Keys
----
b  capture baseline stats (seated ONLY)
...
BASELINE_CAPTURE_DURATION_SEC = 60
BASELINE_CAPTURE_MIN_SAMPLES = 15

# -------------------------- Gap band settings --------------------------
# Adjust so the band (yellow in seam patch preview) covers the interface gap line.
GAP_BAND_Y0_FRAC = 0.46
GAP_BAND_Y1_FRAC = 0.54

# -------------------------- Otsu + morphology params --------------------------
BAND_BLUR_K = 5                 # odd
COL_DARK_RATIO_AT_OR_ABOVE = 0.55   # column considered "gap" if >=30% of pixels are dark
...
def compute_gap_metrics_otsu(seam_patch: np.ndarray):
    ...
    # Blur band to stabilize Otsu
    k = BAND_BLUR_K if BAND_BLUR_K % 2 == 1 else BAND_BLUR_K + 1
    band_blur = cv2.GaussianBlur(band, (k, k), 0)

    # Otsu threshold (binary inverse => dark pixels are 255)
    thr, mask = cv2.threshold(band_blur, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
...
def main():
    ...
    # Optional baseline stats (for reference only)
    ...
    try:
        ...
        while cam.IsGrabbing():
            ...
            # baseline capture (reference only)
            ...
            if cap_active:
                ...
                if (len(cap_bands) >= BASELINE_CAPTURE_SAMPLES) or (elapsed >= BASELINE_CAPTURE_DURATION_SEC):
                    ...
                    if len(cap_bands) >= BASELINE_CAPTURE_MIN_SAMPLES:
                        ...
                        band_stats = {"median": med, "mad": mad, "y0_frac": GAP_BAND_Y0_FRAC, "y1_frac": GAP_BAND_Y1_FRAC}
                        cfg["baseline_gap_band_stats"] = band_stats
                        save_config(cfg)
                        print(f"[BASELINE] Saved gap band stats (reference): median={med:.1f} mad={mad:.1f}")
                    cap_bands = []

            # Detection metrics (Otsu)
            if (not cap_active) and (not unstable) and seam_patch is not None:
                dark_ratio, max_run, thr, band_y = compute_gap_metrics_otsu(seam_patch)
            ...
            if band_stats is not None:
                cv2.putText(bg, f"Baseline(ref) med={band_stats.get('median',0):.1f} mad={band_stats.get('mad',0):.1f}", (30, 225), FONT, 0.8, txt, 2)
            ...
            cv2.putText(bg, f"otsu_thr={None if thr is None else f'{thr:.0f}'}  inliers={last_orb.get('inliers',0)} ratio={last_orb.get('inlier_ratio',0):.2f}", (30, 335), FONT, 0.75, txt, 2)
            ...
            if show_debug:
                ...
                if seam_patch is not None:
                    ...
                    if band_y is not None:
                        ...
                        if thr is not None:
                            cv2.putText(patch_vis, f"otsu_thr={thr:.0f}", (5, max(15, y0+18)), FONT, 0.6, (0, 255, 255), 2)
