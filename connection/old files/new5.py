#!/usr/bin/env python3
"""orb_homography_inspection_v17_unstable_hysteresis.py

Connector seating inspection using ORB homography + seam patch profile matching.

What this version adds (requested):
- UNSTABLE hysteresis to stop flicker between GOOD and UNSTABLE near the threshold.
    * Enter UNSTABLE when ScoreMed <= SCORE_UNSTABLE_ENTER
    * Exit UNSTABLE only when ScoreMed >= SCORE_UNSTABLE_EXIT
- Slightly longer score median window (SCORE_HISTORY) to reduce jitter.

Behavior:
- GOOD (green): stable pose + no defect indicators
- UNSTABLE - REPOSITION (orange): pose mismatch / tracking lost (does NOT latch)
- BAD (red): defect indicators (shift/residual/hard limits); BAD latches if PerPartLatch=True

Baseline capture:
- Timeboxed capture: 25 samples over 90 seconds, min 20 samples
...
# Hard shift fail (immediate BAD)
HARD_SHIFT_FAIL_AT_OR_ABOVE = 12

# History smoothing
SHIFT_HISTORY = 7
SCORE_HISTORY = 9   # increased to reduce flicker
RESID_HISTORY = 7
...
# Enter UNSTABLE when score is at/below this
SCORE_UNSTABLE_ENTER = 0.90
# Exit UNSTABLE only when score is at/above this
SCORE_UNSTABLE_EXIT  = 0.95
...
def best_shift_signed(base_prof: np.ndarray, cur_prof: np.ndarray, max_shift: int) -> tuple[int, float]:
    """Return (signed_shift, score) maximizing normalized dot product."""
    w = len(base_prof)
...
def main():
    ...
    # UNSTABLE state with hysteresis
    unstable = False
    unstable_reason = ""
    ...
    try:
        ...
        while cam.IsGrabbing():
            ...
            if not paused:
                if state == "WAIT_FOR_PART":
                    ...
                    if present_ct >= ENTER_PRESENT_FRAMES:
                        ...
                        # reset non-latched statuses on new part
                        unstable = False
                        unstable_reason = ""
            ...
            if k in (ord('b'), ord('B')) and (not cap_active):
                ...
                # During baseline capture: clear statuses, do not evaluate
                bad = False
                part_failed = False
                unstable = False
                unstable_reason = ""
            ...
            if (not cap_active) and base_profile is not None and prof is not None and len(base_profile) == len(prof):
                ...
                # Update histories (require fresh H)
                if H_is_fresh:
            ...
            # ------------ UNSTABLE hysteresis update ------------
            # Determine tracking / pose stability (does NOT latch)
            if (not cap_active) and state == "INSPECT" and base_profile is not None:
                ...
                if not tracking_ok:
                    ...
                    # Pose mismatch based on score with hysteresis
                ...
                # No baseline or not inspecting -> not unstable
                if state != "INSPECT":
                    unstable = False
                    unstable_reason = ""

            # ------------ Decision (BAD only when NOT unstable) ------------
            prev_bad = bool(bad)
