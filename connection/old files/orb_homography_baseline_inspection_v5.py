#!/usr/bin/env python3
"""orb_homography_baseline_inspection_v5.py

ORB + Homography + Baseline Boundary (v5)
---------------------------------------
This version fixes the usability problem you hit:
  "Cannot set baseline: not stable yet" repeatedly.

Why that happened
-----------------
In v4, baseline capture was gated by StableCount, which only increments after several
consecutive frames meet a jitter criterion. If the boundary reading is intermittent
or competing edges exist, StableCount can hover at 0 even though BoundaryMed is visible.

What v5 changes
---------------
1) Baseline capture is now an explicit *capture session*:
   - Press 'b' to start capturing boundary readings for N frames.
   - The script computes the median of the collected boundary_x values and saves it.
   - This removes dependence on StableCount for baseline capture.

2) Stability counting is still used for per-part "stable max-gap" behavior, but it's
   made a bit more forgiving (smaller history window requirement).

3) Keeps prior robustness:
   - ORB+RANSAC homography, hold-last-good.
   - Consistent quad ordering per frame.
   - Tightened post-baseline search window (BASELINE_MARGIN_RIGHT=60).
   - Template (ORB anchor) quad overlay in debug.
   - On-screen toast messages.

Keys
----
  b : start baseline capture session (collects for BASELINE_CAPTURE_FRAMES)
  u : clear baseline
  l : toggle latch mode
...
BOUNDARY_HISTORY = 9
MIN_HISTORY_FOR_JITTER = 3   # was effectively 5; make more forgiving
JITTER_PX_MAX = 6            # allow small wobble
STABLE_REQUIRED = 3          # fewer stable ticks
JUMP_REJECT_PX = 25
...
def run(camera, converter, cfg):
    ...
    # Baseline capture session state
    ...
    toast("Press u to clear baseline. Press b to start baseline capture (collects ~18 frames).")
    ...
    while camera.IsGrabbing():
        ...
        if k in (ord('u'), ord('U')):
            ...
            baseline_capture_left = 0
            baseline_samples.clear()
            toast("Baseline cleared. Press b to capture baseline")
        ...
        if state == "INSPECT" and use_H is not None:
            ...
            if seam_patch is not None:
                ...
                # stability tick (for stable max-gap)
                ...
                if baseline_capture_active:
                    ...
                    if baseline_capture_left <= 0:
                        ...
                        if len(baseline_samples) >= BASELINE_CAPTURE_MIN_SAMPLES:
                            ...
                            save_config(cfg)
                            toast(f"Baseline captured (median) X={baseline} using {len(baseline_samples)} samples")
                            # clear tracking after capture
                            boundary_hist.clear()
                            prev_boundary = None
                            stable_count = 0
                        else:
                            toast(f"Baseline capture failed: only {len(baseline_samples)} samples. Try again.")
                        baseline_samples.clear()

                if baseline is not None and boundary_med is not None:
                    gap_px = int(max(0, boundary_med - baseline))

        # Start baseline capture session
        if k in (ord('b'), ord('B')):
            if state != "INSPECT":
                ...
                baseline_samples.clear()
                toast(f"Baseline capture started: collecting {BASELINE_CAPTURE_FRAMES} frames...")
        ...
        if baseline_capture_active:
            cv2.putText(bg, f"Baseline capture: {BASELINE_CAPTURE_FRAMES-baseline_capture_left}/{BASELINE_CAPTURE_FRAMES} frames", 
