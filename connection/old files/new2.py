#!/usr/bin/env python3
"""orb_homography_inspection_v13_baseline_capture_safe.py
...
- SAFE BASELINE CAPTURE:
    * while capturing baseline, the script does NOT evaluate GOOD/BAD
    * BAD latch/counters/history are cleared when capture starts
    * UI shows "CAPTURING BASELINE..." with frames remaining
- Sanity checks:
...
# Match quality gates (catch "shift stays 0 but connector changed")
...
def main():
    ...
    try:
        ...
        while cam.IsGrabbing():
            ...
            # Do not evaluate GOOD/BAD while capturing a baseline
            if (not cap_active) and state == "INSPECT" and (not paused) and base_profile is not None and shift_med is not None:
                ...
                # Optional clearing (kept from original logic)
                if (
                    (not per_part_latch)
                    and clear_on_reseat
            ...
            if base_profile is None or cap_active:
                bg[:] = (0, 165, 255)
                if cap_active:
                    label = f"CAPTURING BASELINE... ({cap_left} frames left)"
                else:
            ...
            cv2.putText(bg, label, (30, 120), FONT, 1.6, txt, 5)
