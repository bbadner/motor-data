#!/usr/bin/env python3
"""orb_homography_inspection_v16_unstable_reposition.py

Purpose
-------
Connector seating inspection using ORB homography + seam patch profile matching.

This version implements your requested behavior (Option B):
- If pose/position/rotation causes a poor correlation to the baseline, show
  **UNSTABLE / REPOSITION** (orange) instead of latching BAD.
- Only latch **BAD** (red) for true defect indicators (shift/residual) and
  hard failures.

Baseline capture
----------------
Timeboxed baseline capture suitable for ~96 s takt:
- 25 samples spread across 90 seconds
- minimum 20 samples required to save
- NO GOOD/BAD decisions while capturing baseline
...
- "BAD" latches if PerPartLatch=True.
"""
...
# If correlation drops below this, treat as pose mismatch -> UNSTABLE (not BAD)
SCORE_UNSTABLE_AT_OR_BELOW = 0.92
# Optional: if residual is extremely high, treat as defect even if score is low
RESID_FAIL_AT_OR_ABOVE = 0.65
RESID_CONSEC_TO_TRIP = 3

# Hard fails (immediate BAD) - keep conservative
    ...
    try:
        ...
        while cam.IsGrabbing():
            ...
            if not paused:
                if state == "WAIT_FOR_PART":
                    ...
                    if present_ct >= ENTER_PRESENT_FRAMES:
                        ...
                        # entering inspect resets unstable
                        unstable = False
                        unstable_reason = ""
                        # entering inspect resets non-latched states
            ...
            if k in (ord('b'), ord('B')) and (not cap_active):
                ...
                # While capturing baseline, clear status and do not evaluate
                bad = False
                part_failed = False
                unstable = False
                unstable_reason = ""
            ...
            if cap_active:
                ...
                if (len(cap_profiles) >= BASELINE_CAPTURE_SAMPLES) or (elapsed >= BASELINE_CAPTURE_DURATION_SEC):
                    ...
                    cap_profiles = []
                    # clear histories after baseline
                    bad = False
                    part_failed = False
                    unstable = False
                    unstable_reason = ""
            ...
            # ------------ Metrics (only when not capturing) ------------
            ...
            # Determine tracking / pose stability
            if (not cap_active) and state == "INSPECT":
                # Tracking missing (no homography or no profile)
                if (use_H is None) or (prof is None) or (base_profile is None):
                ...
                # Pose mismatch (score low) -> UNSTABLE, not BAD
                elif score_med is not None and score_med <= SCORE_UNSTABLE_AT_OR_BELOW:
                    unstable = True
                    unstable_reason = "REPOSITION"

            # BAD decisions only when NOT unstable and we have metrics
            if (not cap_active) and (not paused) and state == "INSPECT" and (not unstable) and base_profile is not None and shift_med is not None:
                ...
                # Optional clearing (kept behavior)
                if (
                    (not per_part_latch)
                    and clear_on_reseat
                ...
                # Per-part latch only for BAD (not UNSTABLE)
            ...
            cv2.putText(bg, f"ScoreMed({SCORE_HISTORY}): {None if score_med is None else f'{score_med:.3f}'}  Unstable<= {SCORE_UNSTABLE_AT_OR_BELOW}",
