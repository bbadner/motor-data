...
def get_page_text(doc: fitz.Document, pdf_file: str, page_num: int) -> str:
    ...
    Returns the best text blob found, scoring by useful content rather than raw length
    so that a rotated page's garbage output doesn't beat the correctly-oriented text.
    """
    ...
    def _score(s: str) -> int:
        """
        Score a candidate by useful signal rather than raw character count.
        Boost candidates that contain recognisable words from the report header
        (date keywords, month names, the company name, measurement terms).
        This prevents upside-down OCR garbage from outscoring the correct rotation.
        """
        useful_keywords = [
        ...
        lower = s.lower()
        keyword_hits = sum(1 for kw in useful_keywords if kw in lower)
        # Each keyword hit is worth 200 chars — enough to prefer a shorter but
        # correctly-oriented candidate over a longer garbage one.
        return base + keyword_hits * 200
