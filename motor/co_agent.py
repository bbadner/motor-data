...
def read_input_power(filepath):
    ...
    # Choose the correct engine: openpyxl for .xlsx/.xlsm, xlrd for legacy .xls
...
def create_motor_pdf(output_path, months, month_values, labels=None):
    ...
    # Use provided labels, or fall back to month strftime
    ...
    for label, vals in zip(month_labels, month_values):
        ...
        pdf.cell(0, 8, f"{label}: Mean={mu:.2f} W, Median={med:.2f} W, StdDev={sd:.4f}", ln=True)
...
def run_motor_analysis(q=None, cancel_flag=None):
    """
    Process motor Excel files and generate trend analysis PDF.

    Rules applied:
    1. DEDUPLICATION — if the same date appears in both a .xls and a .xlsx file
       (same filename stem, different extension), keep only the .xlsx copy.
       If two files have truly identical dates AND identical stems, keep one.
    2. PER-FILE labeling — every file is its own data point on the chart.
       Multiple files in the same calendar month are labeled "Jul 1", "Jul 2", etc.
    3. LAST 12 FILES — after dedup and sort, take the 12 most-recent files
       (not 12 months). This means the chart always shows exactly 12 boxes.
    """
    ...
    # ── Step 2: deduplicate ──────────────────────────────────────────────────
    # Group by (date, stem-without-extension).
    # If a stem has both .xls and .xlsx, keep only the .xlsx.
    # If identical (date+stem+ext) appears twice, keep one.
    from collections import defaultdict
    stem_groups = defaultdict(list)   # key: (date, stem) -> list of filenames
    ...
    for (d, stem), fnames in stem_groups.items():
        # Prefer .xlsx over .xls; if only one, use it
        xlsx = [fn for fn in fnames if fn.lower().endswith(".xlsx")]
        chosen = xlsx[0] if xlsx else fnames[0]
        deduped.append((d, chosen))

    deduped.sort(key=lambda x: x[0])
    print(f"[INFO] Motor files after dedup: {len(deduped)}")
    for d, f in deduped:
        print(f"  {d.strftime('%Y-%m-%d')}  {f}")

    # ── Step 3: take the last 12 files ──────────────────────────────────────
    recent = deduped[-12:]

    # ── Step 4: read power values for each file ──────────────────────────────
    file_entries = []   # list of dicts: {date, filename, label, values}
    for d, f in recent:
        ...
        if vals:
            ...
            print(f"[WARN] No power values read from: {f}")

    if not file_entries:
        print("❌ No motor Input Power data found.")
        return

    # ── Step 5: build per-file labels (month counter resets each month) ──────
    month_counters = defaultdict(int)
    for entry in file_entries:
        mk = entry["date"].strftime("%Y-%m")
        month_counters[mk] += 1
        count = month_counters[mk]
        month_name = entry["date"].strftime("%b %Y")
        # Only add a number suffix when this month appears more than once
        entry["label"] = f"{month_name} {count}" if count > 1 or month_counters[mk] > 1 else month_name

    # Second pass: any month that ultimately has >1 file needs the "1" suffix too
    final_month_counts = defaultdict(int)
    for entry in file_entries:
        final_month_counts[entry["date"].strftime("%Y-%m")] += 1

    month_seen = defaultdict(int)
    for entry in file_entries:
        mk = entry["date"].strftime("%Y-%m")
        month_seen[mk] += 1
        if final_month_counts[mk] > 1:
            entry["label"] = f"{entry['date'].strftime('%b %Y')} {month_seen[mk]}"
        else:
            entry["label"] = entry["date"].strftime("%b %Y")
