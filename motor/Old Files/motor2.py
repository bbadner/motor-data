# ============================================================
# Rexair Motor & Magnet Analysis – CLEAN SCRIPT v12
# ============================================================
...
print("RUNNING CLEAN SCRIPT v12")
...
def read_input_power(path):
    ...
    for v in df.values.flatten():
        ...
        try:
            ...
            # Motor input power normally around 600–750
            if 500 < x < 900:
                vals.append(x)
...
def run_motor_analysis(q=None,cancel_flag=None):
    ...
    print("[MOTOR] Months used:",
          [m.strftime("%Y-%m") for m in months_dt])
...
def create_magnet_pdf(output_path, results):
    ...
    for i,r in enumerate(results):
        ...
        safe_name=r['file'].encode('latin-1','ignore').decode('latin-1')

        cp=(USL-LSL)/(6*r["std"])

        pdf.cell(
            0,8,
            f"{labels[i]} Mean: {r['mean']:.4f} Std: {r['std']:.4f} Cp: {cp:.2f} CpK: {r['cpk']:.2f}",
            ln=True
        )
