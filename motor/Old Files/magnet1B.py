...
def parse_3x_values(text: str) -> List[float]:
    """
    Extract decimal numbers and keep those in the magnet timing range.
    This is more reliable for noisy OCR scans.
    """
    ...
    for m in matches:
        try:
            ...
            # keep only magnet timing values
            if 3.0 <= v <= 4.5:
                values.append(v)
...
def extract_values_from_page2(doc: fitz.Document, pdf_file: str) -> List[float]:
    ...
    # 3) NEW: OCR center-right column (where 3.7° values live)
    w, h = base_img.size

    column_crop = base_img.crop(
        (int(w * 0.55), int(h * 0.10), int(w * 0.85), int(h * 0.95))
    )

    column_proc = preprocess_for_ocr(column_crop)

    for psm in (4, 6, 11):
        text = ocr_image(column_proc, psm=psm)
        vals = parse_3x_values(text)
        if vals:
            candidates.append(vals)
