...
import numpy as np
import pandas as pd
print("[INFO] Running motor_agent6_fixed v1")
...
def extract_timing_values_from_pdf(text: str):
    """Return up to 30 timing values from PDF text.

    For text-based PDFs this works well; for OCR output we apply a fuzzy extractor.
    """
...
def _parse_degree_tokens_to_floats(tokens):
    vals = []
    for t in tokens:
        if t is None:
            continue
        ...
        s = s.replace(',', '.')
        try:
            if re.fullmatch(r'[34]\.\d{2}', s):
                v = float(s)
            elif re.fullmatch(r'[34]\d{2}', s):
                v = float(s) / 100.0
            else:
                continue
            if 3.0 <= v <= 4.5:
...
def _extract_timing_values_fuzzy(raw_text: str, target_count: int = 30):
    ...
    Handles common OCR issues:
      ...
      - dropped dot (e.g., 371 -> 3.71)
      - stray characters

    Returns values in appearance order.
    """
    if not raw_text:
        return []
    ...
    # Normalize common OCR confusions (keep ASCII only)
    ...
    header = re.search(r"3\.7\s*(?:±|\+/-|\+\s*/\s*-|\+\s*/-|\+\s*-|/|-)\s*0\.5", t)
    scan_text = t[header.start():] if header else t

    vals = []

    # First pass: decimals like 3.71
    for m in re.finditer(r"([34]\.[0-9]{1,2})", scan_text):
        try:
            v = float(m.group(1))
        except Exception:
            continue
    ...
    for m in re.finditer(r"([34][0-9]{2})", scan_text):
        s = m.group(1)
        try:
            v = float(s) / 100.0
        except Exception:
...
def extract_timing_values_from_pdf_images(pdf_path, dpi=450):
    ...
    for page_index in range(doc.page_count):
        ...
        # Define crop boxes as fractions (left, top, right, bottom)
        # Try a few variants to handle layout changes.
        if w >= h:
            # landscape-like
            boxes = [
                (0.55, 0.10, 0.95, 0.95),
        ...
        for (lx, ty, rx, by) in boxes:
            ...
            # Binarize with a slightly lower threshold to keep faint digits
