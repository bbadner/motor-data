...
# ---------- LOGGING SETUP ----------
def init_logger():
    """Initialize or append to log.txt"""
...
def create_progress_window(title="Processing"):
    """Creates a progress bar window with a cancel button"""
    win = tk.Toplevel()
    win.title(title)
...
# ---------------------------------------------------
# Section 3: Motor Test Trend Analysis (Updated)
# ---------------------------------------------------
import os
import re
import pandas as pd
...
def run_motor_analysis(q=None, cancel_flag=None):
    ...
    # Determine path for .exe or .py execution
    ...
    for f in excel_files:
        try:
            ...
            # Find sheet containing “High Speed(Open)” section
            found = False
            ...
            # Locate the “Input Power(W)” column
            power_col = None
            for col in df.columns:
                if isinstance(col, str) and "Input Power" in col:
                    power_col = col
                    break
            ...
            # Extract the correct date from filename (after C6521)
    ...
    # Box-and-whisker chart (Average Watts)
...
# ---------------------------------------------------
# Section 4: Updated Motor and Magnet Analysis
# ---------------------------------------------------
# ---------------------------------------------------
# Section: Magnet Timing CpK Analysis
# ---------------------------------------------------
import os
import re
import statistics
...
def run_magnet_analysis(q=None, cancel_flag=None):
    ...
    # Determine working path (handles .exe or .py)
    base_path = os.path.dirname(sys.executable if getattr(sys, "frozen", False) else __file__)
    ...
    for f in pdf_files:
        try:
            ...
            # 🔧 Fix split decimals (e.g. "3.7\n8" → "3.78")
            text = re.sub(r"3\.(\d)\s+(\d)", r"3.\1\2", text)

            # Extract valid float values between 3.0 and 4.5
            ...
            # Extract date from filename (8 digits)
