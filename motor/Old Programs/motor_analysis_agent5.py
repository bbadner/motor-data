...
# --------------------------------------------------
# Embedded Rexair Logo (base64-encoded PNG image)
# --------------------------------------------------
REXAIR_LOGO_BASE64 = """
iVBORw0KGgoAAAANSUhEUgAAAGQAAAAyCAYAAAAMkLmnAAABF0lEQVR4nO3UMQ2AMAwDQaH7/zsNbzFtT2GVcFMuEnZ5gY0PTv7LOAAAAAAAAAMDPD9zX9+VyRgAAAAAAAAAAAAAAAKyRvNmK3ktjgzPy3kvCpk7Z0h/rz2UpHLxzZmbzXZf+ps19UUAAAAAAAAAAAAAAABzzfweGkpGE8PAZ0EAAAAASUVORK5CYII=
"""  # <- shortened for display. Yours should contain the full base64 string

def extract_logo_to_temp():
    try:
        temp_dir = tempfile.gettempdir()
        logo_path = os.path.join(temp_dir, "_temp_rexair_logo.png")
        with open(logo_path, "wb") as f:
            f.write(base64.b64decode(REXAIR_LOGO_BASE64))
        if os.path.exists(logo_path):
            return logo_path
        else:
            print("[WARN] Logo file was not created.")
            return None
    except Exception as e:
        print(f"[ERROR] Failed to decode embedded logo: {e}")
        return None
...
def create_motor_pdf(path, months, all_values, means, stds):
    logo_path = extract_logo_to_temp()
    pdf = FPDF()
    pdf.add_page()
...
def run_motor_analysis(q=None, cancel_flag=None):
    ...
    report_dir = os.path.join(base, "Motor Reports")
...
def run_magnet_analysis(q=None, cancel_flag=None):
    base = os.path.dirname(os.path.abspath(sys.executable if getattr(sys, "frozen", False) else __file__))
    report_dir = os.path.join(base, "Magnet Reports")
    os.makedirs(report_dir, exist_ok=True)
    ...
    for f in pdfs:
        ...
        values = [float(v) for v in matches if v.strip("°").replace(".", "", 1).isdigit()]
        if not values:
