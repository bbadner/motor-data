...
# ---------- PDF: Motor Report ----------
def generate_dynamic_summary(months, means, stds, all_values):
    summary_lines = []

    # Stability (check overall std dev trend)
    if len(stds) > 3:
        if stds[-1] > stds[-2] > stds[-3]:
            summary_lines.append(" Stability: Standard deviation has increased over the past 3 months.")
        elif stds[-1] < stds[-2] < stds[-3]:
            summary_lines.append(" Stability: Standard deviation has decreased over the past 3 months.")
        else:
            summary_lines.append(" Stability: No consistent trend in standard deviation.")

    # Process Centering (mean near target range)
    in_range = [840 <= m <= 845 for m in means]
    percent_centered = sum(in_range) / len(in_range) * 100
    summary_lines.append(f" Process Centering: {percent_centered:.0f}% of months had mean in target range (840–845W).")

    # Variability: track standard deviation change
    max_sd = max(stds)
    min_sd = min(stds)
    summary_lines.append(f" Variability: Std Dev ranged from {min_sd:.2f}W to {max_sd:.2f}W across reported months.")

    # Outliers per month
    outlier_counts = []
    for values, mean, std in zip(all_values, means, stds):
        threshold = 2 * std
        outliers = [v for v in values if abs(v - mean) > threshold]
        outlier_counts.append(len(outliers))
    worst_month = months[outlier_counts.index(max(outlier_counts))].strftime("%b %Y")
    summary_lines.append(f" Outliers: Most outliers occurred in {worst_month}.")

    # Recent Month Comparison
    if len(means) > 1:
        diff = means[-1] - means[-2]
        direction = "increased" if diff > 0 else "decreased"
        summary_lines.append(f" Recent Insight: Latest month’s mean has {direction} by {abs(diff):.2f}W compared to previous.")
...
def create_motor_pdf(path, months, all_values, means, stds):
    ...
    for i, m in enumerate(months):
        ...
        # Page 3: Dynamic Summary
    pdf.add_page()
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 10, "Dynamic Summary of Input Power Trends", ln=True)
    pdf.set_font("Arial", "", 11)
    summary_lines = generate_dynamic_summary(months, means, stds, all_values)
